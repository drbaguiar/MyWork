...
df0<- data.frame(cbind (Ind=rep("White",20),Dep=rep("NotSick",20)))
df1<- data.frame(cbind (Ind=rep("White",5),Dep=rep("Sick",5)))
total <- rbind(df0, df1)
df0<- data.frame(cbind (Ind=rep("Black",10),Dep=rep("NotSick",10)))
df1<- data.frame(cbind (Ind=rep("Black",20),Dep=rep("Sick",20)))
total <- rbind(total, df0)
total <- rbind(total, df1)
df0<- data.frame(cbind (Ind=rep("Hispanic",10),Dep=rep("NotSick",10)))
df1<- data.frame(cbind (Ind=rep("Hispanic",15),Dep=rep("Sick",15)))
total <- rbind(total, df0)
total <- rbind(total, df1)
df0<- data.frame(cbind (Ind=rep("Other",10),Dep=rep("NotSick",10)))
df1<- data.frame(cbind (Ind=rep("Other",10),Dep=rep("Sick",10)))
total <- rbind(total, df0)
total <- rbind(total, df1)
rm(df0,df1)
