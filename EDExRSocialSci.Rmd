...
filename <- file.path("D:/Data/femaleMiceWeights.csv")
...
filename <- "D:/Data/femaleControlsPopulation.csv"
...
Here we'll try a more concise way to do many random samples, using the replicate() function in R. The replicate() function takes two arguments, which are the number of times to replicate, and then an expression: the command you want to replicate. First let's try out making a random sample of one group of 12 mice from the population and calculating the mean:
```{r}
...
abline(h=mean(null), col=2)
# add a line for the DIFF
abline(h=diff,col=3)
# Calculate p-value
mean(null>=diff)
