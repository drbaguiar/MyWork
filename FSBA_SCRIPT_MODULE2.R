...
data=read.table('D:/Data/DATA_2.01_SKU.csv', header = T,sep=',') # The function read.table enables us to read flat files such as .csv files
...
data=read.table('D:/Data/DATA_2.02_HR.csv',header = T,sep=',') # The function read.table enables us to read flat files such as .csv files
...
# As discussed in the quiz, let's remove the Newborn variable, which is not really relevant and by being a dummy drives the clustering too much...
...
write.csv(aggdata, "D:/Data/HR_example_Numerical_Output.csv", row.names=FALSE)
...
# Let's load the data
data=read.table('D:/Data/DATA_2.03_Telco.csv', header = T,sep=',')# The function read.table enables us to read flat files such as .csv files
...
write.csv(aggdata, file = "D:/Data/aggdataTelco5seg.csv", row.names=FALSE) # Let's save the output in a csv to work on it in Excel later
